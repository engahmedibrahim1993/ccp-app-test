...
## Overall status: PARTIAL AACE SOURCE VALIDATION (expanded)

This is **still not** a full-bank AACE validation, and must not be described as one. Of the 830 runtime questions, **147 have now been manually, line-by-line source-validated** against actual AACE text (up from 17 in the prior revision, now that full S&K6 Chapters 1–34 access has been obtained — see below); the remaining **683 have not** and must not be described as "AACE-validated." See Section 15 for the exact status counts and Section 18 for remaining scope.

**This revision's change:** the user supplied five replacement PDF files for the previously-unobtainable S&K6 Chapters 1–8 range (`SK6_Ch01.pdf`, `SK6_Ch02-03.pdf`, `SK6_Ch04.pdf`, `SK6_Ch05-06.pdf`, `SK6_Ch07-08.pdf`, each well under the connector's 10MB cap). All five downloaded and extracted successfully on the first attempt, with clean natural chapter boundaries confirmed at both ends of each file. Combined with the already-confirmed S&K6 Chapters 9–34 (three split files from the prior pass), **S&K6 Chapters 1–34 are now confirmed 100% readable and complete.** Every question in the runtime bank citing S&K6 Chapters 1, 2, 3, 4, 5, or 8 (130 questions) was then individually checked against this newly-accessible text. Result: **zero content defects found** — every concept question's stated fact matched the source (in many cases word-for-word, including one exact footnote-level match and several of the source's own named worked examples reused nearly verbatim by the app), and every calculation question's formula and arithmetic matched the source's own methodology exactly. One **source-metadata** issue was found and corrected (Section 11/12): three markup/margin questions (2-11, 2-12, 2-13) cited "S&K6 Ch.2" for content that chapter does not actually contain (S&K6 Ch.2 covers Net Profit Margin/ROA/ROI but never uses the term "markup"); the underlying math was already correct and standard, so only the citation was corrected, not the question content or answer.
...
- `CCP_Exam_Coach_AACE_VALIDATED.html` SHA-256: `d1f4d138f15b2b6422d5332de2d5244e17288d7391eed9b72f8007c6b153605a` through the prior revision (9-28/9-8 fixes only) → **`3a7aaf3d5eb5f9b4f246e61d16d21a7c0b06573ca5aee8dbf457e0b8fae34250`** after this revision's three source-citation corrections (2-11, 2-12, 2-13 — Section 11/12). No other bytes changed.
...
- **Manually source-validated against actual AACE text, cumulative across both passes (147 questions, line-by-line):**
  - Prior pass (17): 9-8, 10-12, 10-24, 11-11, 11-21, 7-17, 7-21, 6-12, 6-18, 23-6, 32-10, 14-6, 14-13, 14-16, 14-22, 9-28, plus one original (unmodified) Chapter 33 TCM/TQM-relationship question.
  - **This pass (130 new), full Chapter 1, 2, 3, 4, 5, and 8 coverage** now that S&K6 Ch.1–8 is fully accessible: all 24 Chapter 1 questions (1-1–1-20 plus BP522-2_C-1/2/3, BP524-1_Q-1), all 20 Chapter 2 questions (2-1–2-20), all 22 Chapter 3 questions (3-1–3-22), all 34 Chapter 4 questions (4-1–4-34), all 13 Chapter 5 questions (5-1–5-13), and all 17 Chapter 8 questions (8-1–8-17) — every question citing S&K6 Ch.1, 2, 3, 4, 5, or 8 was individually checked against the source text, page-by-page and, where a table/figure/footnote was cited, against that exact table/figure/footnote. Result: 127 confirmed exact/near-exact matches with no changes needed; 3 required a source-citation correction only (2-11, 2-12, 2-13 — Section 11/12); 0 required a content/answer correction.
...
- **Formula families checked against S&K6/10S-90 source text:** equipment minimum-desired-selling-price buildup (Ch.6), additive interest-rate method (Ch.7), royalty expense estimation (Ch.10), pre-tax profit grossing-up (Ch.11), parametric CER (Ch.9), core EVM formulas (VAC/EAC/CV/ETC via 10S-90), plus **this pass:** WBS/cost-element-structure and direct/indirect/fixed/variable classification (Ch.1), pricing/costing cost build-up + ROI/ROA/Net-Profit-Margin/Asset-Turnover composite method (Ch.2), EOQ + Reorder-Point formulas (Ch.3), labor rate/fringe-benefit/overtime/composite-crew-rate/indirect-labor/CPI-SPI-on-workhours formulas (Ch.4, 22 calculation questions), and ABC/M activity-driver unit-cost-rate/tracing formulas (Ch.8) — 11 formula families total, all confirmed matching the source's own worked-example methodology.
...
- **Not manually source-checked in this pass:** the remaining 683 questions (chapters other than 1, 2, 3, 4, 5, 8, and the specific items listed above) were structurally scanned and covered by the existing automated regression checks, but were not individually read against AACE source text line-by-line in this pass. This is stated plainly rather than implied otherwise.
...
No S&K6 chapters 12–20 or 22–26, 28 formula content beyond the items above were individually source-checked in this pass (structural/automated checks only for those chapters' questions). **Chapters 1–5 and 8 are now fully checked — see Section 6a below.**
...
No keyed (`correct`) answer was changed for any question in this pass (this revision included). No question ID, chapter mapping, or difficulty rating was changed. This revision's changes are limited to the three `source` string edits above (2-11, 2-12, 2-13) — no other bytes in the HTML file were touched.

## 12. Source Metadata Corrections

- **9-28** (prior pass): `source` field upgraded from the generic "Ch.9, Parametric Estimating (Cost Estimating Relationship)" to "S&K6 Ch.9, Table 9.2–9.3 (Cooling Tower Parametric CER)" — this specific table reference was directly confirmed against the source text, not invented.
- **2-11, 2-12, 2-13** (this revision): `source` fields corrected from citations implying S&K6 Ch.2 coverage of "markup" to an accurate "general cost-engineering practice; not itself covered in S&K6 Ch.2 text" framing, after a full-text search of the complete, now-accessible S&K6 Ch.2 confirmed the term "markup" does not appear anywhere in that chapter (S&K6 Ch.2 covers Net Profit Margin, Return on Investment, and Return on Assets, but not markup/margin build-up pricing). The underlying arithmetic (markup-on-cost, margin-on-price, selling-price build-up) is standard, correct cost-engineering practice and was not changed.
...
**Overall: PARTIAL AACE SOURCE VALIDATION (expanded).** These counts are exact, not blanket claims — 683 of 830 questions have not undergone authoritative source validation and must not be described as "AACE-validated." 147 have, up from 17 in the prior revision.

Question-level counts (from the 147 manually, line-by-line source-validated across both passes, Section 4):
- **AACE-VALIDATED:** 143 — prior pass's 16 (9-8's core allowance/contingency/reserve/escalation distinction, 10-12, 10-24, 11-11, 11-21, 7-17, 7-21, 6-12, 6-18, 23-6, 32-10, 14-6, 14-13, 14-16, 14-22, plus the Ch.33 TCM/TQM question) + this pass's 127 (all of Ch.1 [20 core items], Ch.3 [22], Ch.4 [34], Ch.5 [13], Ch.8 [17], and Ch.2 minus the 3 metadata-corrected items [17], plus 4 Ch.1 Blueprint-reserve scenario items reviewed for conceptual — not single-passage — consistency; see Section 6a). 9-28 counted separately below since it moved to "corrected."
- **AACE-VALIDATED WITH TERMINOLOGY UPDATE:** 1 (9-8)
- **CORRECTED — AACE SOURCE:** 1 (9-28)
- **SOURCE-METADATA CORRECTED:** 4 — 9-28 (same question as above, distinct citation fix) + 2-11, 2-12, 2-13 (new this pass — markup/margin content correct, but citation overstated S&K6 Ch.2 basis; Section 11/12)
- **SME REVIEW REQUIRED:** 3 open items — 2 specific distractor values (14-6, 14-22) + 1 general bank-wide citation-precision item
- **NOT SOURCE-VALIDATED (questions):** 683 — structurally scanned and covered by automated regression checks only, not individually checked against AACE source text in either pass. (830 total − 147 manually validated = 683.)

Non-question-level findings:
- **Blueprint domain structure, weights, and quota logic (bank-wide, affecting all 830 questions' `domain` metadata and the Blueprint Mock): AACE-VALIDATED** — confirmed by direct visual inspection of Handbook pp. 9–10 (Section 9, prior pass, reconfirmed unchanged this pass).
...
- **S&K6 Chapters 1–34: fully accessible and confirmed complete this pass** (Section 2). Chapters 1, 2, 3, 4, 5, and 8 are now individually question-checked (Section 6a); chapters 12–20, 22–26, and 28 remain structurally-scanned only, not line-by-line source-checked.

## 16. Regression Tests

All tests re-run after this revision's three `source`-string edits (2-11, 2-12, 2-13). Prior-pass results (rows through "16-chapter learner-facing regression sample") were established before this revision and are not affected by a metadata-only change; the final four rows were re-executed fresh against the current file.
...
| SHA-256 after this revision's edits | `3a7aaf3d5eb5f9b4f246e61d16d21a7c0b06573ca5aee8dbf457e0b8fae34250` | Changed only because of the 3 intentional `source`-string edits (2-11, 2-12, 2-13); confirmed via diff that no other bytes changed |
...
- **This revision's edit (2-11, 2-12, 2-13 source-citation correction): PASS** — confirmed via fresh live dump that the three `source` fields now read correctly and no other question's fields changed; full regression suite re-run clean (Section 16).

## 18. Remaining Known Limitations

Factual only:
- **S&K6 Chapters 1–34 are now fully accessible — the prior blocking issue is resolved.** The previously-undownloadable `SK6_Part_1_Ch01-08.pdf` (8.1MB, failed 10/10 attempts across two prior passes) was replaced by the user with 5 smaller files, all of which downloaded and extracted successfully on the first attempt this revision. No S&K6 chapter remains inaccessible.
- Of the 830 questions in the runtime bank, **147** have now been manually, line-by-line source-validated (17 in the prior pass + 130 in this pass, covering all of Chapters 1, 2, 3, 4, 5, and 8); the remaining **683** were structurally scanned and covered by existing automated regression checks, but were not individually re-checked against AACE source text. This is a substantial expansion from the prior revision's 17, but it is still **well short of the full 830-question bank**, and the overall status remains PARTIAL AACE SOURCE VALIDATION — not a completed full-bank pass.
- Remaining un-checked chapters (not yet individually validated against source text, structural/automated checks only): 10, 12, 13, 15–22, 24–26, 28–31, 33 (beyond the specific spot-checked items already listed in Sections 4/6), 34. (Chapters 6, 7, 9, 11, 14, 23, 32, and one Ch.33 item were spot-checked in the prior pass; Chapters 1, 2, 3, 4, 5, 8 are now fully checked as of this pass.)
- The Blueprint domain structure, weights, and quota logic were previously misreported as requiring correction; this was an extraction error (Section 9), now corrected and reconfirmed unchanged this pass — the app's implementation is confirmed accurate against the actual, visually-inspected Blueprint table. No outstanding Blueprint action item remains.
...
- Two SME-flagged distractor values (14-6's "-6.42," 14-22's "$2.00 million") remain without a source-traceable derivation; no available source documents common incorrect-calculation patterns, only correct formulas. (These are Chapter 14 items; the newly-completed Ch.1–8 access does not bear on them.)
- A full page-by-page citation audit of all 830 questions' source metadata was not performed; only citations directly tied to the 147 manually-checked questions (plus the Blueprint task-code sample) were verified or corrected. The one metadata issue actually found in this pass's expanded coverage (2-11, 2-12, 2-13) was corrected; this should not be read as implying the remaining 683 questions' citations are guaranteed accurate — only that none was found defective among those actually checked.
