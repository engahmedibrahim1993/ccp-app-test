...
| — | Method Selection Drill | PARTIALLY TESTED | Excellent 2-stage design: Stage 1 asks you to name the correct method/formula family BEFORE any numbers are shown; committing to a wrong method shows a "Method selection miss" diagnostic with the correct method name, formula, and exact source citation, then proceeds to Stage 2 (solve with fresh answer choices) regardless of Stage 1 outcome. Verified this full mechanic on one complete round (Ch.23 Bonding Capacity: I deliberately picked a wrong Stage-1 method, got the miss diagnostic + citation, then Stage 2 presented correctly). Each "Start drill" draws a new random Q1 from a 277-question pool (does not persist across reload, like Calculation Drill/Memo) which made systematically working through multiple rounds impractical within this session's tooling — genuine mechanic confirmed, but only 1 full round completed rather than a 5-10Q sample. |
| 4 | Calculation Drill (= task's "Generated Practice") | TESTED | 8 dynamic items sampled across Ch.13 (CPM fwd pass x2), Ch.9 (capacity-exponent scaling), Ch.14 (BCWS), Ch.34 (°F→°C), Ch.27 (PV of future sum), Ch.7 (DDB), Ch.11 (RRAT breakeven qty). Independently re-derived and verified all 8 app-stated correct answers — all exactly matched. Honestly self-labeled "DYNAMIC — FRESH NUMBERS" vs "Calculation Practice" (fixed Master Bank questions) — transparent that this is number-substitution into verified templates, not novel scenario generation. Good formula variety, all internally consistent. Note: progress does NOT survive a page reload (unlike Chapter Practice/Mocks) — see persistence finding. |
...
| 15 | Formula Lab | TESTED | 27 formula cards (Ch.2,3,4,6,7,9,10,11,13,19,27,29,30) each with variables, "what it tells you," an explicit **"common trap,"** and a worked example. Standout feature — the Ch.30 LP card's trap ("optimum is always at a vertex, check all corners") is exactly the insight needed for the two LP problems I solved earlier. The Ch.10 breakeven card openly notes a known S&K6 textbook erratum (misprinted equation) rather than silently reproducing it — strong integrity signal. Gap: only 13 of 34 chapters have cards; EV formulas are filed under "CH.19" heading even though Ch.14 is literally "Earned Value Overview" (minor mistagging, logged as content issue) |
...
| 24 | Final Week Mode | TESTED | 20/20 (100%). Draws broadly from calculation-heavy/Hard-tagged content across many chapters rather than truly personalized weak-spot-only content — reasonable given my near-perfect history, but for a low-mistake learner this behaves more like "hard-question drill" than "your specific gaps." Would need a learner with more real mistakes to fully judge the weak-domain-targeting claim. |
| 10 | Adaptive Practice | TESTED | 15/15 (100%) — genuinely well-targeted: several Hard multi-relationship CPM network problems (fully independently verified, matched exactly), both EAC methods, incremental-milestone EV trap, and specifically re-served my one flagged "fragile correct" item (mutually-exclusive probability) from earlier. Classify as **Highly Relevant** recommendations for this learner profile. |
...
| 17 | MCQ Blueprint Mock (Blueprint Practice Mock, 119Q) | PARTIALLY TESTED | Native confirm() dialog gates entry with honest framing ("uses the training bank... does NOT count as cold/unseen readiness evidence"). Sampled 7/119 questions — content spans many chapters/domains with the claimed domain-weighted distribution; some overlap confirmed with questions seen elsewhere in the bank (as the dialog itself warns). Same UI as Timed Practice Mock. **Not completed** (119Q ≈ 3hr, impractical within remaining session budget) — genuinely incomplete, not skipped. **Notable finding**: the countdown timer is wall-clock-based — survived a ~2.5hr real-world session gap and kept counting down in the background rather than freezing, a strong exam-integrity design (logged as a positive finding). |
| 18 | Memo Practice | TESTED | Completed one full realistic memo (Estimate Increase Reconciliation scenario): 30-min timer, live word count, wrote a ~280-word structured memo (To/From/Subject/Purpose/4-driver reconciliation/recommended actions — deliberately omitted an explicit Date line). Self-Review Checklist (10 items, checkboxes) accurately recorded "9/10" after I honestly left "Date" unchecked. Reviewer-rating control explicitly warns "ONLY SET THIS IF A HUMAN... HAS ACTUALLY REVIEWED THIS MEMO" — strong integrity design, resists self-inflated scores. Saved memo correctly appears in Memo History with checklist score. App is explicit throughout that this is not official AACE scoring. |
